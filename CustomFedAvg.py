...
class CustomFedAvg(Strategy):
    ...
    def aggregate_arrayrecords(self,
        records: list[RecordDict], weighting_metric_name: str, model
    ) -> ArrayRecord:
        ...
        # 1. Setup your model, loss function, and optimizer
        device = "cuda" if torch.cuda.is_available() else "cpu"
        criterion = torch.nn.CrossEntropyLoss().to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=self.lr)

        # Inside the training loop (e.g., for epoch in range(num_epochs):)
        for inputs, labels in self.trainloader:
            
            # --- Step 2: Zero the gradients ---
            # Clear previous gradients before a new backward pass, 
            ...
            # Calculate how far off the prediction was from the true label
            loss = criterion(outputs, labels)
            
            # --- Step 5: Backward pass (Gradient Computation) ---
            # This is where PyTorch calculates the gradient of the loss 
            # with respect to every model parameter that requires a gradient.
            loss.backward()

            # --- Step 6: Optimizer step (Gradient Application) ---
            # This applies the computed gradients to update the model parameters (weights/biases).
            optimizer.step()
